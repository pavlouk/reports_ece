...
class ImageCollection:
    ...
    def make_images(cls) -> None:
        ...
        for i, hour in enumerate(experiment["samples"]):
            for mouse_id in experiment["mouse_ids"]:
                for angle in experiment["mouse_angles"]:
                    fi = FLIRImage()
                    jpg_id = experiment[mouse_id][angle]["ir"][i]
                    csv_id = experiment[mouse_id][angle]["csv"][i]
                    dc_id = experiment[mouse_id][angle]["dc"][i]
                    fi.id = jpg_id
                    fi.mouse_id = mouse_id
                    fi.angle = angle
                    fi.sample = hour
                    fi.set_ir(
                        fpath=f"{module_path}/data/raw/{hour}/{mouse_id}/{jpg_id}"
                    )
                    fi.set_csv(
                        fpath=f"{module_path}/data/raw/{hour}/{mouse_id}/{csv_id}"
                    )
                    fi.set_dc(fpath=f"{module_path}/data/raw/{hour}/{mouse_id}/{dc_id}")
