...
Uint32 delayed_input(float time, int position, int *buffer){

Uint32 echo;
int displacement, pos;

displacement = time *48000;
if (displacement == 0) echo = buffer[position];
else {
...
void delay_array_clear(int *BUFFER){
	int i;
	for (i=0;i<N;i++){
		BUFFER[i]=0;
	}
...
while(1) {
	...
	if (prevStatus !=0) switch_status_display(status);
	prevStatus = status;
